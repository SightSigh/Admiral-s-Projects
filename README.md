...
:-------:|:----:|:----------:|:----------:|:------------:|:-------:|:-------:|:-------:|:-------:|
装备改修+星级争霸|POI预装插件|以POI联网进行游戏|持续|√|有|每级纸面数据及装备耗材余量查询，无统计|总体目标|√|
明石工具箱|手机应用|无特别依赖|中止|×|无|仅每级纸面数据，无统计|×|×|
AKASHI-LIST|网页|以浏览器联网|持续|×|无|每级纸面数据，以特定级别为起点应用必成模式计算装备升级理论消耗|×|×|
此表格|XLSX文件|本地兼容EXCEL2010以上软件或微软EXCEL手机应用（OneDrive网页端亦可编辑）|持续|×|不实装|统计改修及升级中装备耗材以外的具体消耗，附加简易可行性验证|具体目标|√|

关于其他工具，KCV不自带改修插件，以前用过的几个已经停更了；74eo没用过不清楚；INT的改修插件的更新速度比本体还慢，可以视为断更，只有POI的改修插件做的最完善。  
...
### 刁钻Q&A
    ...
    Q：炒饭团薅每日羊毛不就得了，需要这么复杂么？  
    A：螺丝只有转化为出击装备的改修等级才能提升实，饭团除外。
    
    Q：那我买足够的螺丝直接毕业不就好了？还用得着记录？  
    A：首先，这游戏基本不存在改修毕业这一说法，如若让每人都享有专属装备，至少需要六位数级别的CNY投入，还不算装备更新换代；  
       其次，不用受制于螺丝可以随意改修的人，也不需要记录，顺带提一下，还需要另外再掏钱买齿轮，它的消耗比螺丝多。
    
    Q：每天一项进度太慢了，我一天想改多项，但项目数不固定，也想做记录，怎么办？  
    A：依照目前的框架，你可以按需扩展表格，比较简单的方法是将每周的完整结构复制多次，后面统计页面修改一下统计位置即可。  
       完美一点可以把结构由横向改为纵向，幅度拉长，减少同一页面中的元素，这样方便浏览。  
       今后也有可能会推出适配每日多项改修的表格，但是目前八字还没一撇。
    ...
    A：在规划时正确选择表格中的装备阶段标签，并运用十以内加减法进行计算即可。
