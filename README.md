...
项目名称|形式|数据更新状况|依存关系|关联个人数据|蓝图模式|消耗统计|记载改修进度|支持换日改修|
:-------:|:----:|:----------:|:----------:|:------------:|:-------:|:-------:|:-------:|:-------:|
装备改修+星级争霸|POI预装插件|续更|以POI联网进行游戏|√|有|每级纸面数据及装备耗材余量查询，无统计|总体目标|×|
明石工具箱（舰百）|手机应用|已断更|手机联网或离线|×|无|仅每级纸面数据，无统计|×|×|
AKASHI-LIST|网页|续更|浏览器联网|×|无|选择特定级别起应用必成模式计算理论消耗|×|×|
此表格|EXCEL文件|续更|本地EXCEL2010以上兼容软件或微软云端EXCEL应用（OneDrive可）|×|未定|统计装备耗材以外的实际消耗，附加可行性验证|具体目标|√|
