...
活用“delete”键来清除内容以及创建当前改修进度的标记，凡是能够下拉单元格都可以运用此方法  

简易可行性可以参考预期收益表中的日均消耗线  

由于齿轮还可以用于开发、建造，无法由此规划表统计消耗量，故不可作为可行性参考

如果不需要使用可行性预测相关的统计功能，首先清空收益预期表中的计数器，再将可折叠部分的下拉单元格清空并隐藏明细即可

    
## 助手相关说明
换日改修，即在日本时间（UTC+9）零时至四点五十九分之间进行改修，其可改修内容已更换为下一天的项目，本表已经为适配此项设定进行了改造。

* 本表默认使用者拥有所有助手的最高改造形态，故当一个项目出现某助手只能换日改修，但还有其他助手可以进行正常改修时，只能换日改修的助手将不予显示  
    例：15.2cm連装砲 日 能代、~~★矢矧~~ ←不予显示
