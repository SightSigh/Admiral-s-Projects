...
### 刁钻Q&A
    ...
    Q：每日就补贴1个螺丝，有什么用？  
    A：每年累计补贴365个螺丝，价值25550日元，按成文时中期市场汇率16.32计算，相当约1565人民币。
    
    Q：炒饭团薅每日羊毛不就得了，需要这么复杂么？  
    A：将螺丝转化为出击时携带装备的改修等级才能确实提升实力，饭团除外，因为它是消耗品。
       炒饭只有一个值得做的原因：解放装备位。除此之外，这么做只能造成价值转换的亏损，装备成型周期的推迟等负面作用。
    ...
    Q：每天改修一项进度太慢了，我希望一天改修多项，但项目数不固定，也想做记录，怎么办？  
    A：依照目前的框架，你可以按需扩展表格，比较简单的方法是将每周的完整结构复制多次，后面统计页面修改来源的单元格即可。  
    ...
    A：按照授权你可以自行演绎此项目，做的好一点，这样我就不用继续维护这个简陋的表格啦。
    
    Q：我看不懂规划表，能不能加个指示器来显示每个装备计划开始的时候是几星，结束时是几星？
    A：在制订规划时正确选择表格中的装备阶段标签，并运用十以内加减法进行口算即可得知。
       由于此表格跟游戏实际数据无法直接关联，进行完整的指示增加的操作过多而没多大用处，故不设此指示。
